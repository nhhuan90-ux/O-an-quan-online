/**
 * Game IQ Suite - Main Coordinator
...
class IQAudio {
  ...
  playChime() {
    ...
    // Play a lovely major chord (E5, G#5, B5, E6)
  ...
  playVictory() {
    ...
    this.init();
    const now = this.ctx.currentTime;
    // Triumphant brass melody: C4 -> E4 -> G4 -> C5 -> E5 -> G5 -> C6
    const notes = [
      { f: 261.63, d: 0.1 },
      { f: 329.63, d: 0.1 },
      { f: 392.00, d: 0.1 },
...
class GameManager {
  ...
  restartGame() {
    if (this.undoStack.length > 0) {
      // Revert to first state (initial state)
      ...
      // Clear stacks but keep initial state
      this.undoStack = [];
      this.redoStack = [];
      this.saveMoveState(); // save new initial
  ...
  saveMoveState() {
    ...
    // Increment moves if timer has started
  ...
  triggerFirstMove() {
    if (!this.timerStarted) {
      ...
      this.startTimer();
      // Save initial state before first modification
      if (this.undoStack.length === 0) {
        this.saveMoveState();
      }
    }
  ...
  /* Victory Fireworks Animation */
  startFireworks() {
    this.stopFireworks();
    
    // Match canvas size to overlay
    ...
    const animate = () => {
      ...
      // Auto spawn sparks from bottom randomly
      if (Math.random() < 0.1) {
      ...
      for (let i = this.fireworks.length - 1; i >= 0; i--) {
        ...
        this.victoryCtx.shadowBlur = 6;
        this.victoryCtx.shadowColor = p.color;
...
// Global click sound helper for game cells
...
// Start the game manager on DOM load
